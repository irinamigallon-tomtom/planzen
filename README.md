...
**Per-week capacity:** The input may also include week columns labelled in `D.M.` format (e.g. `30.3.`, `6.4.`) or `D-Mon` format (e.g. `30-Mar`, `6-Apr`). When the `Engineer Capacity (Bruto)` row has values in week columns, those override the scalar `Estimation` value week-by-week. Partial coverage is accepted — missing weeks fall back to the scalar derived from the available weeks' mean. Absence is lenient: missing weeks default to 0. See `LOGIC.md` for details.
